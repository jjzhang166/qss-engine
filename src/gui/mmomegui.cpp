...
MmomeGui::_run (QString name)
{
  ...
  env.insert ("MMOC_ENGINE",
	      QDir (_utils->appDir (MMOC_ENGINE)).absolutePath ()); // Add an environment variable
  env.insert ("MMOC_LIBS", QDir (_utils->appDir (MMOC_LIBS)).absolutePath ()); // Add an environment variable
  env.insert ("MMOC_MODELS",
	      QDir (_utils->appDir (MMOC_MODELS)).absolutePath ()); // Add an environment variable
	  ...
	  if (c->checkState () == Qt::Checked)
	    {
	      ...
	      SLASH).append (_model->item (k)->text ()).append (SLASH).append (
		  c->text ()).append (".dat\"");
	      c = it->child (g, 2);
	      if (c->text () != "None")
		{
...
MmomeGui::_comp_finished (int exitCode, QProcess::ExitStatus exitStatus)
{
  if (exitStatus == QProcess::NormalExit)
      ...
      if (!makefi.exists ())
	{
      ...
      QProcessEnvironment env = QProcessEnvironment::systemEnvironment ();
      env.insert ("MMOC_BUILD",
		  QDir (_utils->appDir (MMOC_BUILD)).absolutePath ()); // Add an environment variable
      env.insert ("MMOC_BIN", QDir (_utils->appDir (MMOC_BIN)).absolutePath ()); // Add an environment variable
      _proc->setProcessEnvironment (env);
...
MmomeGui::on_actionLog_triggered ()
{
  ...
  QProcessEnvironment env = QProcessEnvironment::systemEnvironment ();
  env.insert ("MMOC_OUTPUT",
	      QDir (_utils->appDir (MMOC_OUTPUT)).absolutePath ());
...
outputDir.cd (Editor::instance ()->activeBaseFileName ());
QFileInfo logfi (
  outputDir.absolutePath () + SLASH + Editor::instance ()->activeBaseFileName ()
      + QString (".log"));
if (logfi.size ())
{
