...
VarInfo_::VarInfo_ (Type t, AST_TypePrefix tp, AST_Modification m,
		    AST_Comment c) :
    _state (false), _discrete (false), _t (t), _tp (tp), _m (m), _comm (c), _builtin (
	false), _index (), _size (0), _value (0), _algebraic (false), _exp (
...
VarInfo_::VarInfo_ (Type t, AST_TypePrefix tp, AST_Modification m,
		    unsigned int s, bool array) :
    _state (false), _discrete (false), _t (t), _tp (tp), _m (m), _comm (NULL), _builtin (
	false), _index (), _size (s), _value (0), _algebraic (false), _exp (
    NULL), _hasStart (false), _hasEach (false), _hasAssigment (false), _name (), _isArray (
	array)
{
  _processModification ();
      ...
      else if (t == MODCLASS)
	{
	  ...
	  if (asgExp->expressionType () != EXPNULL)
	      ...
	      if (!isParameter ())
		{
		  foreach(it,al)
		    {
		      if (current_element(it)->argumentType ()
			  == AR_MODIFICATION)
			{
			  AST_Argument_Modification am =
			  current_element(it)->getAsModification ();
			  if (am->modification ()->modificationType ()
			      == MODEQUAL)
			    {
			...
			list<Index> idxs, int constant, int forOffset)
{
  ...
  if (vi->hasEachModifier ()
      && (_vste == VST_INIT || _vste == VST_CLASSIC_INIT))
    {
      stringstream tmp;
      tmp << "[" << idx;
      if (!vi->isParameter ())
    ...
    {
      idxStr = printIndex (vi->index (), idxs, idx, offset, constant, vi,
			   order);
    }
...
string
VarSymbolTable_::printIndex (Index idx, list<Index> idxs, string sub,
			     int offset, int constant, VarInfo vi, int order)
{
  stringstream buffer;
