...
ConvertToGiNaC::ConvertToGiNaC (VarSymbolTable varEnv, bool forDerivation, MMO_Expression exp) :
        _forDerivation (forDerivation), _varEnv (varEnv), _replaceDer (true), _generateIndexes (false), _exp (exp)
{
}
...
ConvertToGiNaC::getSymbol (AST_Expression_Derivative der)
{
    ...
    string s = cr->name ();
    s.insert (0, "__der_");
    AST_Expression_ComponentReference new_cr = AST_Expression_ComponentReference_Add (newAST_Expression_ComponentReference (), newAST_String (s),
