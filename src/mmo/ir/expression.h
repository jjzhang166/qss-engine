...
private:
  ...
  void
  _addAlgebriacDeps (Index algIndex, MMO_Equation equation, Index equationIndex,
  			 Index derivativeIndex, Dependencies derivativeDeps,
  			 map<Index, Index> *states,
  			 map<Index, Index> *discretes, Index variableChange, int value);
  void
...
private:
  ...
  _getIndexList (AST_Expression_ComponentReference cr, Index index,
		 list<Index> *idxs);
  string
  _printIndex(Index idx, list<Index> idxs, string sub,
      int offset, int constant);
  VarSymbolTable _vt;
  string _idx;
  int _offset;
  int _order;
 ...
 */
MMO_PrintExp
newMMO_PrintExp (VarSymbolTable vt, MMO_ReplaceInterval ri,
		 MMO_PackageTable pt);
/**
 *
 * @param m
