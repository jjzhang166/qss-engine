...
{
  list<string> ret;
  for (map<string, string>::iterator it = mdeps.begin (); it != mdeps.end ();
      it++)
    {
...
void
MMO_Model_::_setAlgebraic (AST_Expression left, AST_Expression right, int begin,
			   int end, AST_Expression arguments)
{
  _data->clear ();
  AST_Expression_ComponentReference cr = left->getAsComponentReference ();
  string varName = _getComponentName (cr);
...
MMO_Model_::_insertEquation (AST_Equation eq, int begin, int end)
{
  if (end < begin)
    {
      Error::getInstance ()->add (eq->lineNum (), EM_IR | EM_UNKNOWN_ODE,
				  ER_Error, "Wrong equation range.");
    }
  _controlEquation (eq);
  if (eq->equationType () == EQEQUALITY)
      ...
      if (eqe->left ()->expressionType () == EXPDERIVATIVE)
	{
	  ...
	  AST_Expression_ComponentReference edcr = AST_ListFirst (
	      ed->arguments ())->getAsComponentReference ();
	  Index idx, lhs;
	  if (edcr->hasIndexes ())
	    {
    ...
    {
      Error::getInstance ()->add (eq->lineNum (), EM_IR | EM_UNKNOWN_ODE,
				  ER_Error, "Equation type not recognized.");
    }
}
...
MMO_Model_::_insertAlgebraicEquation (AST_Equation eq, int begin, int end)
{
  ...
  if (eq->equationType () == EQEQUALITY)
      ...
      else if (eqe->left ()->expressionType () == EXPOUTPUT)
	{
	  ...
	  foreach(it,el)
	    {
	      _setAlgebraic (current_element(it), eqe->right (), begin, end,
			     eqe->left ());
	    }
	}
      else
  ...
  if (st->statementType () == STASSING)
    {
      ...
      if (transform.empty ())
	{
	  AST_Statement retSt = newAST_Statement_Assign (
	      st->getAsAssign ()->lhs (), r);
	  return (retSt);
	}
      int rValue;
  ...
  else if (st->statementType () == STIF)
    {
      ...
      AST_StatementList stet = _transformStatementList (
	  sti->else_statements ());
  ...
  else if (st->statementType () == STWHEN)
    {
      ...
      AST_StatementList stList = _transformStatementList (
	  stWhen->statements ());
      AST_Statement_ElseList stElseList = _transformStatementElse (
	  stWhen->else_when ());
      AST_Statement retWhen = newAST_Statement_When (stWhen->condition (),
						     stList, stElseList,
						     stWhen->comment ());
      return (retWhen);
    }
  ...
  if (r->expressionType () == EXPBINOP)
	  ...
	  || bo->binopType () == BINOPELEXP)
	{
	  ...
	  if (right->expressionType () == EXPCOMPREF
		...
		{
		  VarInfo lvi = _variableLookup (cleft->name (), cleft,
						 ER_Fatal);
		  VarInfo rvi = _variableLookup (cright->name (), cright,
						 ER_Fatal);
		  if (rvi->size () != lvi->size () && rvi->isArray ()
		      && lvi->isArray ())
		    {
		  ...
		  VarInfo elvi = _variableLookup (eleft->name (), eleft,
						  ER_Fatal);
		  if (lvi->isArray () > 1 && rvi->isArray ())
		      ...
		      if (rvi->size () != elvi->size ())
			{
		      ...
		      _declarations->insert (
			  "i",
			  newVarInfo (newType_Integer (), TP_FOR, NULL, NULL, 1,
				      false));
		      stringstream buffer;
		      buffer << "for i in 1:" << elvi->size () << " loop"
			  << endl;
		...
		{
		  VarInfo lvi = _variableLookup (cright->name (), cright,
						 ER_Fatal);
		  VarInfo elvi = _variableLookup (eleft->name (), eleft,
						  ER_Fatal);
		  if (lvi->size () != elvi->size ())
...
{
  for (list<AST_Statement>::iterator st = _stms.begin (); st != _stms.end ();
      st++)
    {
...
MMO_Model_::initOutput ()
{
  ...
  for (list<AST_Expression>::iterator it = outputs.begin ();
      it != outputs.end (); it++)
    {
...
MMO_Function_::insert (VarName n, VarInfo vi, DEC_Type type)
{
  ...
  if (vi->typePrefix () & TP_CONSTANT)
    {
      vi->setValue (
	  e.foldTraverse (vi->modification ()->getAsEqual ()->exp ()));
    }
  _declarations->insert (n, vi);
  if (type == DEC_PUBLIC)
    {
...
MMO_Function_::localDeclarations ()
{
  ...
  for (VarInfo vi = _localDeclarations->begin (); !_localDeclarations->end ();
      vi = _localDeclarations->next ())
    {
      if (vi->isConstant ())
	{
	  continue;
	  ...
	  || vi->hasEachModifier ())
	{
	  ...
	  Util::getInstance ()->setData (_data);
	  buffer
	      << Util::getInstance ()->printInitialAssignment (vi, "",
							       indexVar);
	  ret.push_back (buffer.str ());
	  buffer.str ("");
	  _data->setSymbols (_declarations);
