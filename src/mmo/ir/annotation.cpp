    ...
    _annotations (), _derivative (), _include ()
{
  _libraryDirectory = Util::getInstance ()->environmentVariable (
      "MMOC_LIBRARIES");
  _includeDirectory = Util::getInstance ()->environmentVariable (
      "MMOC_INCLUDE");
  _annotations.insert (
	...
	ANT_DT_Fixed), _dtSynchString ("SD_DT_Asynchronous"), _desc (), _patohSettings (), _scotchSettings (), _metisSettings ()
{
  ...
  _annotations.insert (
      pair<string, MMO_ModelAnnotation_::type> ("MMO_LPS", LPS));
  _annotations.insert (
...
void
MMO_ModelAnnotation_::_processAnnotation (string annot,
					  ...
					  MMO_AnnotationValue e2, BinOpType bot)
{
  ...
  switch (bot)
    ...
    case BINOPLOWEREQ:
      _setBoolean (
	  (e1.integer () <= e2.integer ()) || (e1.real () <= e2.real ()), &av);
      break;
    case BINOPGREATER:
      _setBoolean ((e1.integer () > e2.integer ()) || (e1.real () > e2.real ()),
		   &av);
