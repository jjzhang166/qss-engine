        ...
        _exp (NULL), _data (data), _str (), _equationIndex (0), _expressionOrder (2), _deps (NULL), _gen (NULL), _printer (NULL), _ri (NULL)
{
    ...
    if (_data->calculateAlgebraics ())
    {
        ...
        _traverseAlgebraics (_deps, _data->lhs (), _deps, &states, &discretes, Index (), DEP_ALGEBRAIC_DEF, -1);
        _traverseAlgebraics (_deps, _data->lhs (), _deps, &states, &discretes, Index (), DEP_ALGEBRAIC_VECTOR_DEF, -1);
    }
...
void
MMO_Expression_::_addAlgebriacDeps (Index algIndex, MMO_Equation equation, Index equationIndex, Index derivativeIndex, Dependencies derivativeDeps,
                                    map<Index, Index> *states, map<Index, Index> *discretes, Index variableChange, int value)
{
    ...
    {
        if (stateVariables.find (*idx) == stateVariables.end () || equation->lhs ().variableChange (algIndex))
        {
            Index stateIndex = Index (idx->applyVariableChange (equation->lhs ().variableIndex (algIndex)));
            ...
            else if (idx->hasRange () && !eqIndex.hasRange ())
            {
                ...
                if (derivativeIndex.hasRange ())
                {
                    ...
                    derivativeDeps->insert (stateIndex.indexValue (stateIndex.value (algIndex.constant ())), newEqIndex, DEP_ALGEBRAIC_STATE);
                }
                else
                {
                    derivativeDeps->insert (stateIndex.indexValue (stateIndex.value (algIndex.constant ())), eqIndex, DEP_ALGEBRAIC_STATE);
                }
            }
            else if (!idx->hasRange () && !eqIndex.hasRange ())
            {
    ...
    {
        if (discreteVariables.find (*idx) == discreteVariables.end () || equation->lhs ().variableChange (algIndex))
        {
            Index discreteIndex = Index (idx->applyVariableChange (equation->lhs ().variableIndex (algIndex)));
            ...
            {
                // X[N] = B[i];
                derivativeDeps->insert (discreteIndex.indexValue (discreteIndex.value (discreteIndex.constant ())), eqIndex, DEP_ALGEBRAIC_DISCRETE);
            }
            else if (!idx->hasRange () && !eqIndex.hasRange ())
            {
                // X[N] = B[N];
    ...
    for (Index *idx = deps->begin (DEP_DISCRETE_VECTOR); !deps->end (DEP_DISCRETE_VECTOR); idx = deps->next (DEP_DISCRETE_VECTOR))
    {
        derivativeDeps->insert (*idx, DEP_DISCRETE_VECTOR);
    }
    _traverseAlgebraics (deps, derivativeIndex, derivativeDeps, states, discretes, variableChange, DEP_ALGEBRAIC_DEF, value);
    _traverseAlgebraics (deps, derivativeIndex, derivativeDeps, states, discretes, variableChange, DEP_ALGEBRAIC_VECTOR_DEF, value);
}

void
MMO_Expression_::_traverseAlgebraics (Dependencies deps, Index derivativeIndex, Dependencies derivativeDeps, map<Index, Index> *states,
    ...
    for (Index *idx = deps->begin (type); !deps->end (type); idx = deps->next (type))
    {
        ...
        for (algEq = algEqs.begin (); algEq != algEqs.end (); algEq++)
        {
            ...
            if (is.type () != IDX_DISJOINT)
            {
                ...
                _addAlgebriacDeps (algKey, (*algEq), equationIndex, derivativeIndex, derivativeDeps, states, discretes, variableCh, algValue);
            }
        }
    }
}
...
string
MMO_Expression_::print (string idx, int offset, int order, int constant, int expressionOrder, int forOffset)
{
    _printer->setEnvironment (idx, offset, order, constant, _expressionOrder, forOffset);
...
MMO_EvalInitExp_::foldTraverseElement (AST_Expression exp)
{
    ...
    switch (exp->expressionType ())
    {
        case EXPCOMPREF:
        {
            ...
            if (!vi->isConstant ())
            {
                Error::getInstance ()->add (exp->lineNum (), EM_IR | EM_INIT_EXP, ER_Error, "Only constants allowed inside initial expressions. %s",
                                            cr->name ().c_str ());
                return (ret);
            }
            return (vi->value ());
        ...
        default:
            ...
            EM_IR | EM_INIT_EXP,
                                        ER_Warning, "Initial expression not recognized, returning zero as default value.");
            break;
    }
    return (ret);
}
...
MMO_EvalInitExp_::foldTraverseElement (int l, int r, BinOpType bot)
{
    switch (bot)
    {
        ...
        case BINOPDIV:
            ...
            {
                Error::getInstance ()->add (0, EM_IR | EM_INIT_EXP,
                                            ER_Warning, "Initial expression zero division, returning zero as default value.");
            }
            break;
        case BINOPMULT:
            return (l * r);
...
AST_Expression
MMO_ConvertCondition_::foldTraverseElement (AST_Expression l, AST_Expression r, BinOpType bot)
{
    switch (bot)
    {
        case BINOPLOWER:
...
MMO_PrintExp_::MMO_PrintExp_ (VarSymbolTable vt, MMO_ReplaceInterval ri, MMO_PackageTable pt) :
        _vt (vt), _idx (), _offset (0), _order (0), _constant (-1), _functions (), _variables (), _expressionOrder (1), _ri (ri), _pt (pt), _forOffset ()
{
}
...
void
MMO_PrintExp_::setEnvironment (string idx, int offset, int order, int constant, int expressionOrder, int forOffset)
{
    _idx = idx;
    _offset = offset;
    _order = order;
...
void
MMO_PrintExp_::_getIndexList (AST_Expression_ComponentReference cr, Index index, list<Index> *idxs)
{
...
{
    list<string> ret;
    for (map<string, string>::iterator it = _variables.begin (); it != _variables.end (); it++)
    {
...
{
    list<string> ret;
    for (map<string, list<string> >::iterator it = _functions.begin (); it != _functions.end (); it++)
    {
        ...
        case EXPCALL:
        {
            ...
            if (fn != BIF_NONE)
            {
                ...
                if (_functions.find (map) == _functions.end ())
                {
                    ...
                    list<string> code = bif->generateCode (map, vn, vars, _expressionOrder);
                    if (code.empty ())
                    {
                        Error::getInstance ()->add (exp->lineNum (),
                        EM_IR | EM_VARIABLE_NOT_FOUND,
...
string
MMO_PrintExp_::_printIndex (Index idx, list<Index> idxs, string sub, int offset, int constant)
{
    stringstream buffer;
    /* if (idxs.empty ())
     {
...
MMO_ReplaceInterval_::_indexExp (int constant, int factor)
{
    ...
    {
        AST_Expression ret = newAST_Expression_BinOp (newAST_Expression_Integer (factor), idx, BINOPMULT);
        return (ret);
    }
    else if (constant != 0 && factor == 1)
    {
        AST_Expression ret = newAST_Expression_BinOp (newAST_Expression_Integer (constant), idx, BINOPADD);
        return (ret);
    }
    else if (constant != 0 && factor != 1)
    {
        AST_Expression ret = newAST_Expression_BinOp (newAST_Expression_BinOp (newAST_Expression_Integer (factor), idx, BINOPMULT),
...
MMO_ReplaceInterval_::foldTraverseElement (AST_Expression exp)
{
    switch (exp->expressionType ())
    {
        case EXPCOMPREF:
        {
            ...
            if (cr->hasIndexes ())
            {
                ...
                if (et == EXPRANGE)
                {
                    ...
                    if (count == 2)
                    {
                        _setIndex (range[0] - vi->index ().low () - 1, 1, range[1] - range[0] + 1, vi);
                        ret->append (newAST_String (cr->name ()), newAST_SimpleList (_indexExp (range[0] - vi->index ().low () - 1)));
                    }
                    else
                    {
                        if (range[1] > 0)
                        {
                            _setIndex (range[0] - vi->index ().low () - 1, range[1], (range[2] - range[0]) / range[1] + 1, vi);
                        }
                        else
                        {
                            Error::getInstance ()->add (exp->lineNum (), EM_IR | EM_FOR_DEF, ER_Fatal, "Range interval not valid");
                        }
                        ret->append (newAST_String (cr->name ()), newAST_SimpleList (_indexExp (range[0] - vi->index ().low () - 1, range[1])));
                    }
                    return (ret);
                }
                else if (et == EXPCOLON)
        ...
        case EXPCALL:
        {
            ...
            switch (Util::getInstance ()->checkBuiltInFunctions (name))
            {
                ...
                case BIF_MAX:
                    ...
                    {
                        Error::getInstance ()->add (exp->lineNum (), EM_IR | EM_VARIABLE_NOT_FOUND, ER_Error, "%s, expected 1 argument, found %d",
                ...
                case BIF_INNER_PRODUCT:
                    ...
                    {
                        Error::getInstance ()->add (exp->lineNum (), EM_IR | EM_VARIABLE_NOT_FOUND, ER_Error, "%s, expected 2 arguments, found %d",
                                                    name.c_str (), AST_Length (exp->getAsCall ()->arguments ()));
                        return (newAST_Expression_Call (newAST_String (name), NULL, nel));
                    }
                    break;
        ...
        case EXPNAMEDARGUMENT:
        {
            ...
            return (newAST_Expression_NamedArgument (e->name (), foldTraverse (e->expression ())));
        }
        default:
            return (exp);
    }
...
AST_Expression
MMO_ReplaceInterval_::foldTraverseElement (AST_Expression l, AST_Expression r, BinOpType bot)
{
...
MMO_ReplaceInterval_::fixedInterval (string var)
{
    ...
    for (list<VariableInterval>::iterator it = _replacedExpsVars[var].begin (); it != _replacedExpsVars[var].end (); it++)
    {
...
{
    for (map<string, AST_Expression>::iterator it = _replacedExps.begin (); it != _replacedExps.end (); it++)
    {
        EqualExp eq (_vt);
...
AST_Expression
MMO_ReplaceInnerProduct::foldTraverseElement (AST_Expression l, AST_Expression r, BinOpType bot)
{
    if (bot == BINOPMULT)
    {
