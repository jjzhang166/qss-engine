...
MMO_MicroModelicaIR_::MMO_MicroModelicaIR_ (string name) :
    _class (NULL), _father (NULL), _child (NULL), _classModification (false), _compositionElement (
	false), _storedDefinition (NULL), _funcs (1), _childPrefix(), _fatherName(), _childName(), _initialCode(),
	_className(), _fatherPrefix(), _classPrefix(), _elseWhen(false)
{
...
MMO_MicroModelicaIR_::_insertComponent (AST_Element_Component x)
{
  ...
  foreachReverse(it,dl)
      ...
      if (array)
	{
	  ...
	  size = e.foldTraverse (
	      AST_ListFirst (current_element(it)->indexes ()));
	}
      ...
      if (tp & TP_CONSTANT)
	{
	  _class->insert (
	      ...
	      newVarInfo (newType_Integer (), tp,
			  current_element(it)->modification (), NULL, size, array),
	      t);
	}
      else
	{
