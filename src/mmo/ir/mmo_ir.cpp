...
MMO_MicroModelicaIR_::MMO_MicroModelicaIR_ (string name) :
        _class (NULL), _father (NULL), _child (NULL), _classModification (false), _compositionElement (false), _storedDefinition (NULL), _funcs (1), _childPrefix (), _fatherName (), _childName (), _initialCode (), _className (), _fatherPrefix (), _classPrefix (), _elseWhen (
                false)
{
...
MMO_MicroModelicaIR_::visit (AST_Class x)
{
    ...
    if (_father != NULL)
    {
        if ((p & CP_FUNCTION) || (p & CP_IMPURE) || (p & CP_PURE))
        {
            ...
            {
                _child->getAsFunction ()->setFunctions (_father->getAsModel ()->functions (), _externalFunctions,
...
MMO_MicroModelicaIR_::visit (AST_Modification x)
{
    ...
    {
        Error::getInstance ()->add (x->lineNum (), EM_AST | EM_CLASS_DEFINITION, ER_Error, _className->c_str (), "Assign modifier.");
    }
