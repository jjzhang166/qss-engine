...
MMO_PackageData_::MMO_PackageData_ (string name,
				    MMO_FunctionDefinitionTable fit,
...
{
  list<string> ret;
  for (MMO_FunctionDefinition fd = _fit->begin (); !_fit->end ();
      fd = _fit->next ())
    {
      if (fd->hasLibraries ())
	{
...
MMO_FunctionDefinition_::MMO_FunctionDefinition_ (string name,
						  string includeDir,
						  string libraryDir,
