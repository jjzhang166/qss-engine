...
__attribute__((hot))
void
QSS_SEQ_stateStep (QSS_simulator simulator)
{
  int i, j;
  double elapsed;
  ...
  QA_recomputeNextTimes (quantizer, nSD, SD[index], t, nextStateTime, x, lqu,
			 q);
  if (nOutputs)
    {
      if (output->nSO[index])
...
__attribute__((hot))
void
QSS_SEQ_eventStep (QSS_simulator simulator)
{
  int i, j;
  double elapsed;
  // Local data structure mappings.
	  ...
	  for (i = 0; i < nRHSSt; i++)
	    {
	      ...
	      if (elapsed > 0)
		{
		  tmp1[infCf0] = evaluatePoly (infCf0, elapsed, q, qOrder);
		}
	      else
		{
...
void
QSS_SEQ_inputStep (QSS_simulator simulator)
{
  int j;
  double elapsed;
  // Local data structure mappings.
...
QSS_SEQ_integrate (SIM_simulator simulate)
{
  ...
  while (t < ft)
	...
	{
	case ST_State:
	  QSS_SEQ_stateStep (simulator);
	  break;
	case ST_Input:
	  QSS_SEQ_inputStep (simulator);
	  break;
	case ST_Event:
	  QSS_SEQ_eventStep (simulator);
	  break;
	default:
	  break;
	}
