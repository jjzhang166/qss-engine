...
BTR_updateS (SC_scheduler scheduler, QSS_data simData, QSS_time simTime)
{
  ...
  BTR_updateTree (scheduler,scheduler->state->BTR_states, simData->SD[stateVar], simData->nSD[stateVar],
		  stateVar, simTime->nextStateTime);
...
BTR_updateH (SC_scheduler scheduler, QSS_data simData, QSS_time simTime)
{
  ...
  if (simTime->type == ST_State)
	...
	BTR_updateTree (scheduler,scheduler->state->BTR_events, simData->SZ[stateVar],
			simData->nSZ[stateVar], -1, simTime->nextEventTime);
	...
	BTR_updateTree (scheduler,scheduler->state->BTR_states, simData->HD[stateVar],
			simData->nHD[stateVar], -1, simTime->nextStateTime);
	  ...
	  for (i = 0; i < simData->event[stateVar].nLHSSt; i++)
	    {
	      ...
	      BTR_updateTree (scheduler,scheduler->state->BTR_states, simData->SD[j], simData->nSD[j], j,
			      simTime->nextStateTime);
	    }
	}
      minIdx = scheduler->state->BTR_states->nodes[scheduler->state->BTR_states->height][0].value;
      minVal = simTime->nextStateTime[minIdx];
      BTR_updateTree (scheduler,scheduler->state->BTR_events, simData->HZ[stateVar],
		      simData->nHZ[stateVar], stateVar, simTime->nextEventTime);
      if (simTime->nextEventTime[scheduler->state->BTR_events->nodes[scheduler->state->BTR_events->height][0].value]
	  < minVal)
	{
	  minIdx = scheduler->state->BTR_events->nodes[scheduler->state->BTR_events->height][0].value;
...
BTR_updateI (SC_scheduler scheduler, QSS_data simData, QSS_time simTime)
{
  ...
  if (simTime->type == ST_State)
    ...
    {
      BTR_updateTree (scheduler,scheduler->state->BTR_states, NULL, 0, simData->TD[simTime->minIndex],
		      simTime->nextStateTime);
      minIdx = scheduler->state->BTR_states->nodes[scheduler->state->BTR_states->height][0].value;
      minVal = simTime->nextStateTime[minIdx];
      BTR_updateTree (scheduler,scheduler->state->BTR_inputs, NULL, 0, simTime->minIndex,
...
BTR_init (SC_scheduler scheduler, QSS_data simData, QSS_time simTime)
{
  ...
  if (simData->events)
    {
      scheduler->state->BTR_events = BTR_Tree (simData->events, simTime->nextEventTime,
			       simTime->weights);
