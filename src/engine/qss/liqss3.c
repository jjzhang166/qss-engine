...
#endif
{
  ...
  if (quantizer->state->flag4[var])
      ...
      else
	{
      ...
      if (a[var] != 0 && (fabs (x[cf3]) > 1e-10)
	  && !quantizer->state->flag3[var] && !quantizer->state->flag2[var])
	{
	  double coeff[3];
	  coeff[0] = a[var] * a[var] * a[var] * q[cf0]
	      + a[var] * a[var] * u0[var] + a[var] * u1[var] + 2 * u2[var];
	  coeff[1] = a[var] * a[var] * a[var] * q[cf1]
	      + a[var] * a[var] * u1[var] + a[var] * 2 * u2[var];
	  coeff[2] = a[var] * a[var] * a[var] * q[cf2]
	      + a[var] * a[var] * u2[var];
