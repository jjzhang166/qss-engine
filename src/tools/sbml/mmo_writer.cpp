...
MMOWriter::MMOWriter (string file) :
    _indent (1), _modelName (), _out (file.c_str (),
				      ios::out | ios::in | ios::trunc), _outputs (), _condition (
	false), _initialAsignment (false)
{
  if (!_out.good ())
    {
...
MMOWriter::visit (MMODecl *x)
{
  ...
  if (x->isCondition ())
    {
      _condition = true;
      _out << utils->indent (_indent) << "if " << x->getExp () << " then"
	  << endl;
      _indent++;
    }
  if (x->isState ())
    {
      if (x->hasValue ())
	{
	  _out << utils->indent (_indent) << "Real " << x->getId ()
  ...
  if (x->isImport ())
    {
      _out << utils->indent (_indent) << "import " << x->getId () << ";"
	  << endl;
    }
}
