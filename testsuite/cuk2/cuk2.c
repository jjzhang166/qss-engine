...
MOD_definition(int i, double *x, double *d, double *alg, double t, double *dx)
{
	...
	{
		case 0:
			alg[0] = (__PAR_L*x[3]+__PAR_L*x[9])/__PAR_L1;
			alg[3] = (d[(1)]*(x[9]+alg[0])-x[0])/(d[(0)]+d[(1)]);
			dx[1] = (alg[3]-x[9])/__PAR_C1;
			return;
		case 1:
			dx[1] = (__PAR_U+x[6]-x[0])/__PAR_L;
			return;
		case 2:
...
MOD_dependencies(int i, double *x, double *d, double *alg, double t, double *der, int *map)
{
	switch(i)
	{
		case 0:
			alg[0] = (__PAR_L*x[3]+__PAR_L*x[9])/__PAR_L1;
			alg[3] = (d[(1)]*(x[9]+alg[0])-x[0])/(d[(0)]+d[(1)]);
			der[0 + 1] = (alg[3]-x[9])/__PAR_C1;
			der[3 + 1] = (__PAR_U+x[6]-x[0])/__PAR_L;
			der[9 + 1] = (-x[6]-alg[3]*d[(0)])/__PAR_L;
			return;
		case 1:
			alg[0] = (__PAR_L*x[3]+__PAR_L*x[9])/__PAR_L1;
			alg[3] = (d[(1)]*(x[9]+alg[0])-x[0])/(d[(0)]+d[(1)]);
...
QSS_initializeDataStructs(QSS_simulator simulator)
{
	...
	int i;
	simulator->data = QSS_Data(4,5,3,0,2,"cuk2");
...
SD_output modelOutput = simulator->output;
	...
	SD_allocOutputMatrix(modelOutput,4,5);
		...
		sprintf(modelOutput->variable[1].name,"iL1");
		sprintf(modelOutput->variable[2].name,"uC");
		sprintf(modelOutput->variable[3].name,"uC1");
		cleanVector(outputs,0,4);

		modelOutput->SO[3][states[3]++] = 0;
		modelOutput->OS[0][outputs[0]++] = 3;
