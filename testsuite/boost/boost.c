...
MOD_definition(int i, double *x, double *d, double *alg, double t, double *dx)
{
	...
	{
		case 0:
			alg[0] = (d[(1)]*x[3]-x[0])/(d[(0)]+d[(1)]);
			dx[1] = (alg[0]-x[0]/__R)/__C;
			return;
		case 1:
			alg[0] = (d[(1)]*x[3]-x[0])/(d[(0)]+d[(1)]);
			dx[1] = (__U-d[(1)]*(x[3]-alg[0]))/__L;
			return;
	}
}
...
MOD_dependencies(int i, double *x, double *d, double *alg, double t, double *der, int *map)
{
	switch(i)
	{
		case 0:
			alg[0] = (d[(1)]*x[3]-x[0])/(d[(0)]+d[(1)]);
			der[0 + 1] = (alg[0]-x[0]/__R)/__C;
			der[3 + 1] = (__U-d[(1)]*(x[3]-alg[0]))/__L;
			return;
		case 1:
			alg[0] = (d[(1)]*x[3]-x[0])/(d[(0)]+d[(1)]);
			der[0 + 1] = (alg[0]-x[0]/__R)/__C;
...
QSS_initializeDataStructs(QSS_simulator simulator)
{
	...
	int i;
	simulator->data = QSS_Data(2,5,3,0,1,"boost");
