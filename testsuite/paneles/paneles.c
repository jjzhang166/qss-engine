...
MOD_definition(int i, double *x, double *d, double *alg, double t, double *dx)
{
	...
	switch(i)
	{
		case 10:
			for(j2 = 0; j2 <= 1; j2++)
			{
					alg[(j2+10) * 3] = (x[(j2) * 3]-x[30])/(__Rshunt+d[(j2+18)]);
			}
			alg[42] = -(__VlineaRef-x[30])-x[33];
	for(j3 = 0;j3 < 3; j3++)
	{
		tmp1[j3] = 0;
	}
	for(j3 = 0; j3 <= 1; j3++)
	{
		...
		tmp1[2] += alg[(j3+10) * 3 + 2];
			...
			dx[1] = (tmp1[0]-__iLoad-alg[42])/__Cbatery;
			return;
		case 11:
			dx[1] = __VlineaRef-x[30];
			return;
		default:
			j = i;
			if(j >=0 && j <= 1)
			{
				...
				dx[1] = (alg[(j+6) * 3]-x[(j) * 3]/__R-(x[(j) * 3]-x[30])/(__Rshunt+d[(j+18)]))/__Cboost;
			}
			j = i-2;
			if(j >=0 && j <= 1)
			{
				alg[(j) * 3] = x[(j+2) * 3];
				alg[(j+2) * 3] = (log(d[(j+16)]*(36*d[(j+10)]-alg[(j) * 3])/(36*d[(12)])+1.0)*__K*__Ac*d[(13)]/__q-alg[(j) * 3]*__Rspv/36)*36;
				alg[(j+4) * 3] = alg[(j+2) * 3];
				alg[(j+6) * 3] = ((d[(j+4)])*x[(j+2) * 3]-x[(j) * 3])/((d[(j+2)])+(d[(j+4)]));
				dx[1] = (alg[(j+4) * 3]-d[(j+4)]*(x[(j+2) * 3]-alg[(j+6) * 3]))/__Lboost;
			}
			j = i-4;
			if(j >=0 && j <= 1)
			{
			...
			if(j >=0 && j <= 1)
			{
				alg[(j+10) * 3] = (x[(j) * 3]-x[30])/(__Rshunt+d[(j+18)]);
				alg[(j+12) * 3] = x[30]*alg[(j+10) * 3];
				dx[1] = alg[(j+12) * 3]/__TContMaxPot;
			}
			j = i-8;
			if(j >=0 && j <= 1)
			{
...
MOD_dependencies(int i, double *x, double *d, double *alg, double t, double *der, int *map)
{
	...
	switch(i)
	{
		case 10:
			...
			for(j6 = 0; j6 <= 1; j6++)
	{
			alg[(j6+10) * 3] = (x[(j6) * 3]-x[30])/(__Rshunt+d[(j6+18)]);
	}
			for(j = 0;j <= 1; j++)
			{
				j5 = j;
			...
			for(j = 0; j <= 1; j++)
			{
				der[(j) * 3 + 1] = (alg[(j+6) * 3]-x[(j) * 3]/__R-(x[(j) * 3]-x[30])/(__Rshunt+d[(j+18)]))/__Cboost;
			}
			for(j = 0; j <= 1; j++)
			{
				der[(j+6) * 3 + 1] = alg[(j+12) * 3]/__TContMaxPot;
			}
	for(j3 = 0;j3 < 3; j3++)
	{
		tmp1[j3] = 0;
	...
	for(j3 = 0; j3 <= 1; j3++)
	{
		...
		tmp1[2] += alg[(j3+10) * 3 + 2];
			...
			der[30 + 1] = (tmp1[0]-__iLoad-alg[42])/__Cbatery;
			der[33 + 1] = __VlineaRef-x[30];
			break;
		case 11:
			for(j7 = 0; j7 <= 1; j7++)
	{
			alg[(j7+10) * 3] = (x[(j7) * 3]-x[30])/(__Rshunt+d[(j7+18)]);
	}
			alg[42] = -(__VlineaRef-x[30])-x[33];
	for(j3 = 0;j3 < 3; j3++)
	{
	...
	if(j >=0 && j <= 1)
	{
		...
		for(j4 = 0; j4 <= 1; j4++)
	{
		alg[(j4+10) * 3] = (x[(j4) * 3]-x[30])/(__Rshunt+d[(j4+18)]);
	}
		alg[(j+12) * 3] = x[30]*alg[(j+10) * 3];
		alg[42] = -(__VlineaRef-x[30])-x[33];
		der[(j) * 3 + 1] = (alg[(j+6) * 3]-x[(j) * 3]/__R-(x[(j) * 3]-x[30])/(__Rshunt+d[(j+18)]))/__Cboost;
		der[(j+2) * 3 + 1] = (alg[(j+4) * 3]-d[(j+4)]*(x[(j+2) * 3]-alg[(j+6) * 3]))/__Lboost;
		der[(j+6) * 3 + 1] = alg[(j+12) * 3]/__TContMaxPot;
	for(j3 = 0;j3 < 3; j3++)
	{
		tmp1[j3] = 0;
	...
	if(j >=0 && j <= 1)
	{
		...
		der[(j) * 3 + 1] = (alg[(j+6) * 3]-x[(j) * 3]/__R-(x[(j) * 3]-x[30])/(__Rshunt+d[(j+18)]))/__Cboost;
		der[(j+2) * 3 + 1] = (alg[(j+4) * 3]-d[(j+4)]*(x[(j+2) * 3]-alg[(j+6) * 3]))/__Lboost;
		der[(j+8) * 3 + 1] = alg[(j+2) * 3]/__Tboost[(j)];
	}
}
...
QSS_data modelData = simulator->data;
	...
	for(i8 = 0; i8 <= 1; i8++)
	{
		__TContMaxPot = 5.000000000000000000000000e-01;
		...
		modelData->d[(12)] = __Ior*pow((modelData->d[(13)]/__Tref),3.0)*exp(__q*__Eg*(1.0/__Tref-1.0/modelData->d[(13)])/__K/__Ac);
	for(i10 = 0; i10 <= 1; i10++)
	{
		modelData->d[(i10+10)] = (__Isc+__K1*(modelData->d[(13)]-__Tref))*modelData->d[(i10+14)]/100.0;
	}
	for(i = 0; i <= 1; i++)
	{
		modelData->nDS[i] = 2;
