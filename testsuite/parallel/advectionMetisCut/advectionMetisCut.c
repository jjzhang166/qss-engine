...
MOD_definition(int i, double *x, double *d, double *alg, double t, double *dx)
{
	...
	switch(i)
	{
		case 0:
			...
			dx[2] = (-10000*x[1]+(-1.0+x[0])*(__alpha-x[0])*__mu*x[1]-(-1.0+x[0])*__mu*x[1]*x[0]+(__alpha-x[0])*__mu*x[1]*x[0])/2;
			return;
		default:
			j = i;
			if(j >=1 && j <= 9999)
			{
				...
				dx[2] = (-x[(j) * 3]*__mu*x[(j) * 3 + 1]*(-1.0+x[(j) * 3])-(x[(j) * 3]-__alpha)*__mu*x[(j) * 3 + 1]*(-1.0+x[(j) * 3])-(x[(j) * 3]-__alpha)*x[(j) * 3]*__mu*x[(j) * 3 + 1]+10000*(x[(j-1) * 3 + 1]-x[(j) * 3 + 1]))/2;
			}
	}
}
	...
	if (map[0] != NOT_ASSIGNED)
{
			...
			der[0 + 2] = (-10000*x[1]+(-1.0+x[0])*(__alpha-x[0])*__mu*x[1]-(-1.0+x[0])*__mu*x[1]*x[0]+(__alpha-x[0])*__mu*x[1]*x[0])/2;
	}
			break;
	}
	j = i+1;
	...
	if (map[j] != NOT_ASSIGNED)
{
		...
		der[(j) * 3 + 2] = (-x[(j) * 3]*__mu*x[(j) * 3 + 1]*(-1.0+x[(j) * 3])-(x[(j) * 3]-__alpha)*__mu*x[(j) * 3 + 1]*(-1.0+x[(j) * 3])-(x[(j) * 3]-__alpha)*x[(j) * 3]*__mu*x[(j) * 3 + 1]+10000*(x[(j-1) * 3 + 1]-x[(j) * 3 + 1]))/2;
	}
	}
	j = i;
	if(j >=1 && j <= 9999)
