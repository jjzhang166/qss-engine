...
MOD_definition(int i, double *x, double *d, double *alg, double t, double *dx)
{
	...
	if(j >=0 && j <= 4999)
	{
		...
		dx[2] = (-x[(j) * 4 + 1]*(1.0/(__CAP[(j)]))*(1.0/(__RES[(j)])))/2;
	...
	if (map[j] != NOT_ASSIGNED)
{
		...
		der[(j) * 4 + 2] = (-x[(j) * 4 + 1]*(1.0/(__CAP[(j)]))*(1.0/(__RES[(j)])))/2;
...
QSS_initializeDataStructs(QSS_simulator simulator)
{
	...
	int i;
	int j = 0;
	simulator->data = QSS_Data(5000,20001,20000,0,0,"aircondsMetisVol");
QSS_data modelData = simulator->data;
	...
	simulator->output = SD_Output("aircondsMetisVol",1,20001,5000,NULL,0,0,CI_Step,SD_Memory,MOD_output);
