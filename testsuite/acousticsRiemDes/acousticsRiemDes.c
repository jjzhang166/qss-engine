...
MOD_definition(int i, double *x, double *d, double *alg, double t, double *dx)
{
	...
	switch(i)
	{
		...
		default:
			...
			if(j >=1 && j <= 198)
			{
				dx[1] = __PAR_K0*((-x[(j+200) * 4])+x[(j+199) * 4])*200;
			}
			j = i-200;
			if(j >=1 && j <= 198)
			{
				dx[1] = 1.0/__PAR_rho0*((-x[(j+1) * 4])+x[(j) * 4])*200;
			}
			j = i-400;
			if(j >=1 && j <= 198)
			{
				dx[1] = (__PAR_u0-__PAR_c)*((-x[(j+401) * 4])+x[(j+400) * 4])*200;
			}
			j = i-600;
			if(j >=1 && j <= 198)
			{
				dx[1] = (__PAR_u0+__PAR_c)*((-x[(j+600) * 4])+x[(j+599) * 4])*200;
			}
	}
}
...
MOD_dependencies(int i, double *x, double *d, double *alg, double t, double *der, int *map)
{
	...
	if(j >=1 && j <= 198)
	{
		der[(j+200) * 4 + 1] = 1.0/__PAR_rho0*((-x[(j+1) * 4])+x[(j) * 4])*200;
	}
	j = i-1;
	if(j >=1 && j <= 198)
	{
	...
	if(j >=1 && j <= 198)
	{
		der[(j) * 4 + 1] = __PAR_K0*((-x[(j+200) * 4])+x[(j+199) * 4])*200;
	}
	j = i-200;
	if(j >=1 && j <= 198)
	{
	...
	if(j >=1 && j <= 198)
	{
		der[(j+400) * 4 + 1] = (__PAR_u0-__PAR_c)*((-x[(j+401) * 4])+x[(j+400) * 4])*200;
	}
	j = i-401;
	if(j >=1 && j <= 198)
	{
	...
	if(j >=1 && j <= 198)
	{
		der[(j+600) * 4 + 1] = (__PAR_u0+__PAR_c)*((-x[(j+600) * 4])+x[(j+599) * 4])*200;
	}
	j = i-600;
	if(j >=1 && j <= 198)
	{
