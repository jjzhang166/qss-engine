...
MOD_definition(int i, double *x, double *d, double *alg, double t, double *dx)
{
	...
	{
		case 0:
			alg[0] = (x[0]*d[(1)]-__PAR_U)/(d[(1)]+d[(0)]);
			dx[1] = (-alg[0]*d[(0)]-x[3])/__PAR_L;
			return;
		case 1:
			dx[1] = (x[0]-x[3]/__PAR_R)/__PAR_C;
			return;
	}
}
...
MOD_dependencies(int i, double *x, double *d, double *alg, double t, double *der, int *map)
{
	switch(i)
	{
		case 0:
			alg[0] = (x[0]*d[(1)]-__PAR_U)/(d[(1)]+d[(0)]);
			der[0 + 1] = (-alg[0]*d[(0)]-x[3])/__PAR_L;
			der[3 + 1] = (x[0]-x[3]/__PAR_R)/__PAR_C;
			return;
		case 1:
			alg[0] = (x[0]*d[(1)]-__PAR_U)/(d[(1)]+d[(0)]);
			der[0 + 1] = (-alg[0]*d[(0)]-x[3])/__PAR_L;
...
QSS_data modelData = simulator->data;
	...
	// Initialize model code.
		...
		modelData->d[(1)] = 1.000000000000000081803054e-05;
