...
MOD_handlerPos(int i, double *x, double *d, double *alg, double t)
{
	...
	if(i >= 2100 && i <= 2199)
	{
		d[(2100)] = 1.000000000000000000000000e+10;
	}
}
...
{
	int *discretes = (int*)malloc(2101*sizeof(int));
	int *events = (int*)malloc(2200*sizeof(int));
	...
	int i;
	int j = 0;
	simulator->data = QSS_Data(3000,2101,2200,0,0,"spl_neurons");
QSS_data modelData = simulator->data;
	...
	modelData->event[i+2100].nLHSDsc = 1;
	}
	...
	for(i = 2100; i <= 2199; i++)
	{
		for(j = 0; j <= 99; j++)
		{
			modelData->HZ[i][events[i]++] = j+2000;
		}
	}
		cleanVector(events,0,2200);
	...
	modelData->event[i+2100].LHSDsc[events[i+2100]++] = 2100;
	}
		cleanVector(events,0,2200);
	...
	modelData->event[i+2100].direction = 1;
	modelData->event[i+2100].relation = 2;
	}
	simulator->time = QSS_Time(3000,2200,0,0,ST_Binary,NULL);

	simulator->output = SD_Output("spl_neurons",1,2101,3000,NULL,0,0,CI_Step,SD_Memory,MOD_output);
SD_output modelOutput = simulator->output;

		modelOutput->nOS[0] = 1;
		modelOutput->nSO[999]++;
	SD_allocOutputMatrix(modelOutput,3000,2101);
		cleanVector(states,0,3000);
